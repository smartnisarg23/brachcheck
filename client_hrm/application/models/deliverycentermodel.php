...
class DeliverycenterModel extends CI_Model {

    function __construct() {
        // Call the Model constructor
        parent::__construct();
        $this->table = "deliverycenters";
    }

    function getAllDeliverycenters() {
        ...
        if ($this->session->userdata['remote_user_data']['employee']['e_role_id'] == 1) {
            ...
            $this->db->where("d.d_company_id", $this->session->userdata['remote_user_data']['employee']['e_company_id']);
            $this->db->where("d.d_branch_id", $this->session->userdata['remote_user_data']['employee']['e_branch_id']);
            $this->db->where("d.id", $this->session->userdata['remote_user_data']['employee']['e_deliverycenter_id']);
        }
    ...
    function createDeliverycenter($data) {
        $this->db->insert($this->table, $data);
        return $this->db->insert_id();
    }

    function updateDeliverycenter($id, $data) {
        $this->db->where('id', $id);
        return $this->db->update($this->table, $data);
    }

    function deleteDeliverycenter($id) {
        $this->db->where('id', $id);
        return $this->db->update($this->table, array("d_status" => -1));
    }

    function getBranchDeliverycenters($company_id, $branch_id) {
