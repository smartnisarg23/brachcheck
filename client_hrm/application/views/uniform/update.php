        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>Update uniform</h5>
                </div>
                <div class="ibox-content">
                    <?php echo form_open(base_url('uniform/update/' . $record_id), array("class" => "form-horizontal")); ?>
                    ...
                    <div class="form-group"><label class="col-sm-2 control-label">Gender</label>
                        <div class="col-sm-10">
                            <div class="i-checks"><label> <input type="radio" value="M" id="u_gender_male" name="u_gender" <?=(isset($u_gender) && $u_gender == "M")? "checked='checked'":""?> > <i></i> Male </label></div>
                            <div class="i-checks"><label> <input type="radio" value="F" id="u_gender_female" name="u_gender" <?=(isset($u_gender) && $u_gender == "F")? "checked='checked'":""?> > <i></i> Female </label></div>
                            <div class="i-checks"><label> <input type="radio" value="U" id="u_gender_unisex" name="u_gender" <?=(isset($u_gender) && $u_gender == "U")? "checked='checked'":""?> > <i></i> Unisex </label></div>
                        </div>
                    </div>
                    <div class="hr-line-dashed"></div>
                    <div class="form-group"><label class="col-sm-2 control-label">Attach role</label>
                        <div class="col-sm-10">
                            <select class="form-control m-b" name="role_attach[]" id="role_attach" multiple="multiple">
                                ...
                                <option value="<?=$value['id']?>" <?= (isset($role_attach) && in_array($value['id'], $role_attach)) ? "selected='selected'" : ""; ?> ><?=$value['r_name']?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
